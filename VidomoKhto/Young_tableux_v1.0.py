...
table_counter = 2
while True:
    ans = input("Ще таблицю? (y/Enter - так, все інше ні): ")
    if ans.lower() not in ('', 'y', 'yes'):
        break
    start_tables.append(get_user_table(user_n))

#print("\n" + "="*30)
print(f"Всього отримано {len(start_tables)} таблиці")

show_table(start_tables)

results = [start_tables[0]]
for i in range(len(start_tables) - 1):
    multiply_by = start_tables[i + 1]
    multiply_results = []
    for t in results:
        multiply_result = generate_all_tableaux (user_n, t, multiply_by)
        multiply_results.extend(multiply_result)
    results = clean_tables(finalize_tables(user_n, multiply_results))

final_tables = results
print("\nОтримуємо")
...
print(f"\nОтримуємо\n{d_in[0]} \u2A02", end=" ")
my_dob = d_in[0]
for s in range(len(d_in) - 2):
    print(f" {d_in[s + 1]} \u2A02", end=" ")
    my_dob *= d_in[s + 1]
print(f" {d_in[-1]} =", end="")
my_sum = 0
for jjj in range(len(d_out) - 1):
    print(f" {d_out[jjj]} \u2A01", end=" ")
    my_sum = my_sum + d_out[jjj]
print(f" {d_out[-1]}")
print(my_dob * d_in[-1], end=" = ")
print(my_sum + d_out[-1])
